        ...
        <el-submenu v-for="item in categoryList" :key="item.id" :index="item.id+''">
          <template slot="title">
            <router-link class="nav-link contribute" :to="'/category/'+item.id">
              <span :class="item.categoryIcon" style="color:orange"></span><span
              class="categoryName">{{item.categoryName}}</span>
            </router-link>
          </template>
          <el-menu-item v-if="item.sysCategoryList!=undefined&& item.sysCategoryList.length!=0"
                        v-for="childItem in item.sysCategoryList" :key="childItem.id" :index="childItem.id+''">
            <router-link class="nav-link contribute" :to="'/category/'+childItem.id">
              <span :class="childItem.categoryIcon" style="color:orange"></span><span class="categoryName">{{childItem.categoryName}}</span>
            </router-link>
          </el-menu-item>
        </el-submenu>
        <el-submenu index="0">
          <template slot="title">
            <router-link class="nav-link contribute" :to="'/admin/index'">
              <span class="el-icon-user" style="color:orange"></span><span
              class="categoryName">后台管理</span>
            </router-link>
          </template>
        </el-submenu>
...
<style>
  ...
  .header-container.not-top #nav .el-menu--horizontal > .el-submenu .el-submenu__title {
    height: 40px;
    line-height: 40px;
    transition: all 0.5s ease-out;
  }
  ...
  .header-container.not-top .el-input--small, .header-container.not-top .el-input--small .el-input__icon {
    line-height: 40px;
  }
</style>
