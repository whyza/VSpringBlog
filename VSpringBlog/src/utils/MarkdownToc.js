...
Toc.prototype._collectTitleElements = function () {
  ...
  for (var j = 0; j < this.elChilds.length; j++) {
    this._elChildName = this.elChilds[j].tagName.toLowerCase(); //标签名 h ==
...
Toc.prototype._createTocContent = function () {
  ...
  for (var i = 0; i < this._elTitleElementsLen; i++) {
    ...
    this._elTitleElementName = this._elTitleElement.tagName  //h1 h2 h3 ==
    this._elTitleElementText = this._elTitleElement.innerText
...
Toc.prototype._showToc = function () {
  ...
  if (!this.options.targetId) {
    ...
    var item = document.getElementById(this.options.targetId);
    // item.replaceChild(document.getElementById(this.toc))
    if (document.getElementsByClassName("list")[0]) {
      item.removeChild(document.getElementsByClassName("list")[0])
